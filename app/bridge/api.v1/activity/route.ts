...
import {
  verifyContextToken,
  matchesTrace,
} from '@/services/activity/context-token';

import { touchSession } from '@/services/session/session';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const projectId =
      ...
      (typeof bodyRecord?.projectId === 'string'
        ? bodyRecord.projectId.trim()
        : undefined);
    ...
    const serverRequest = isValidServerIp(
      project.ipAddress,
      requestIp
    );
    ...
    for (const record of Array.isArray(body) ? body : [body]) {
      ...
      if (
        ...
        tokens.some(
          (token) =>
            typeof token !== 'string' ||
            !verifyContextToken(project.projectSecret, token)
        )
      ) {
        return NextResponse.json(
          {
            message: 'Invalid signed context',
          },
          {
            status: 403,
    ...
    const supplied = aliases
      .filter(
        (name) => bodyRecord?.[name] !== undefined
      )
      .map((name) => bodyRecord![name]);
    ...
    if (
      ...
      return NextResponse.json(
        {
          message: 'Invalid or missing signed context',
        },
        {
          status: 403,
          headers: getCorsHeaders(allowedOrigin),
    ...
    if (
      suppliedTraceId !== undefined &&
      (
        typeof suppliedTraceId !== 'string' ||
        !suppliedTraceId ||
        suppliedTraceId.length > 512 ||
        verified.some(
          ({ contextId }) =>
            !contextId ||
            !project.projectSecret ||
      ...
      return NextResponse.json(
        {
          message: 'Trace ID does not match signed context',
        },
        {
          status: 403,
          headers: getCorsHeaders(allowedOrigin),
    ...
    const activityEvents = await Promise.all(
      verified.map(async ({ event, contextId }) => {
        if (
          contextId &&
          typeof suppliedTraceId === 'string'
        ) {
        ...
        const storedContext = contextId
          ? await prisma.analyticsContext.findFirst({
              where: {
                contextId,
                projectId: project.id,
              },
     ...
     * Session tracking
     *
     * The tracker already sends identifierId with activity
     * events. We use that identifier to maintain the current
     * session for this project.
     *
     * This happens before heartbeat filtering so heartbeat
     * requests can keep the session alive even though heartbeat
     * events are not stored as normal Activity records.
     */
    ...
    await Promise.all(
      sessionIdentifiers.map((identifierId) =>
        touchSession({
          identifierId,
          projectId: project.id,
        })
    ...
    const recordableEvents =
      getRecordableActivityEvents(activityEvents);
